    ...
    <section className="mt-12 sm:mt-16">
      ...
      <div className="mt-10 grid gap-4 lg:grid-cols-[1.15fr_1fr_1fr]">
        <div className="relative min-h-[210px] overflow-hidden border-2 border-foreground bg-foreground p-6 text-background sm:p-7">
          ...
          <div className="relative mt-7 flex items-end gap-2"><span className="text-[68px] font-black leading-none tracking-[-0.07em]">{securityScore ?? '—'}</span><span className="mb-2 font-mono text-[11px] text-background/50">/100</span></div>
          <p className="relative mt-3 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.12em] text-background/65"><span className="h-1.5 w-1.5 rounded-full bg-primary" />{lastScan ? 'Last scan protected' : 'No scan yet'}</p>
        </div>
        ...
        <div className="border border-border bg-card p-6 sm:p-7">
          ...
          <div className="mt-7 flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center border border-[#aebe8c] bg-[#eef1e4] text-[#66763e]"><ShieldCheck size={20} /></div><div><p className="text-[18px] font-bold">{lastScan ? 'Monitoring ready' : 'Not connected'}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">Push + PR checks</p></div></div>
          <div className="mt-8 flex items-end justify-between border-t border-border pt-5"><span className="font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">50 checks</span><span className="text-[13px] font-semibold">{lastScan ? 'Active' : 'Ready'}</span></div>
        </div>
      ...
      <div className="mt-10 border-t border-border pt-8">
        <div className="flex items-center justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">Security coverage</p><h2 className="mt-2 text-[22px] font-bold tracking-[-0.03em]">50 checks, one clear signal.</h2></div><GitPullRequest size={20} className="hidden text-muted-foreground sm:block" /></div>
