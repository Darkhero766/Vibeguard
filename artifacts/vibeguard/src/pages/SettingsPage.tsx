...
import {
  Bell, ChevronRight, CircleUserRound, CreditCard, Database,
  Github, KeyRound, LogOut, Monitor, Moon, Palette, ShieldCheck,
  Sun, Trash2, Download, AlertTriangle, Check, ExternalLink,
} from 'lucide-react';
...
function Toggle({ checked, onChange, label }: ToggleProps) {
  return (
    <button
      ...
      className={`relative h-6 w-11 shrink-0 border-2 border-foreground transition-colors ${checked ? 'bg-primary' : 'bg-muted'}`}
    >
      ...
      <div className="border-b border-border bg-background/40 px-5 py-5 sm:px-7">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1 text-[25px] leading-none">{title}</h2>
          </div>
        </div>
      </div>
...
function Row({ label, description, children, danger = false }: { label: string; description?: string; children?: React.ReactNode; danger?: boolean }) {
  return (
    <div className={`flex flex-col gap-4 border-b border-border/70 px-5 py-5 last:border-b-0 sm:flex-row sm:items-center sm:justify-between sm:px-7 ${danger ? 'bg-destructive/[0.035]' : ''}`}>
      <div className="min-w-0">
        <div className={`text-[13px] font-semibold ${danger ? 'text-destructive' : 'text-foreground'}`}>{label}</div>
        {description && <p className="mt-1 max-w-2xl text-[11px] leading-5 text-muted-foreground">{description}</p>}
      </div>
      {children}
    </div>
  );
      ...
      <main className="relative mx-auto w-full max-w-[1080px] px-5 pb-20 pt-12 sm:px-8 sm:pt-16">
        ...
        <div className="relative z-10 flex flex-col gap-8 sm:flex-row sm:items-end sm:justify-between">
          <div className="vg-rise">
            <div className="flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.2em] text-primary"><span className="h-px w-8 bg-primary" />Control center</div>
            <h1 className="mt-4 text-[48px] leading-[.88] sm:text-[64px]">Settings</h1>
            <p className="mt-5 max-w-xl text-[14px] leading-6 text-muted-foreground">Account, security, scan behavior and data controls. Built for the same security-first workflow as the rest of VibeSane.</p>
          </div>
          <div className="relative overflow-hidden border-2 border-foreground bg-[#101111] px-5 py-4 text-[#f4f1ea] shadow-[6px_6px_0_hsl(var(--primary))] sm:min-w-[245px]">
            <div className="absolute -right-7 -top-7 h-20 w-20 rounded-full border border-[#f4c842]/30" />
            <div className="font-mono text-[9px] uppercase tracking-[0.16em] text-[#f4c842]">Account perimeter</div>
            <div className="mt-2 flex items-center gap-2 text-sm font-semibold"><span className="h-2 w-2 rounded-full bg-[#aeca7a] shadow-[0_0_12px_#aeca7a]" />{plan} ACCESS</div>
            <div className="mt-1 truncate font-mono text-[9px] text-[#92958e]">{user.email}</div>
          </div>
        </div>
