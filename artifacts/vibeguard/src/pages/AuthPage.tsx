      ...
      <main className="flex flex-1 items-center justify-center px-5 py-16 sm:px-8">
        <div className="vg-rise w-full max-w-[420px]">
          ...
          <p className="mt-2 text-[14px] text-muted-foreground">
            {mode === 'signup'
              ? 'One free scan included. No credit card required.'
          ...
          <button
            ...
            className="vg-button vg-focus mt-7 flex w-full items-center justify-center gap-2.5 border border-border bg-card py-2.5 text-[13px] font-semibold text-foreground hover:border-primary/50"
          >
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">or</span>
            <span className="h-px flex-1 bg-border" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              <label className="mb-1.5 block font-mono text-[10px] uppercase tracking-[0.12em] text-foreground" htmlFor="password">
                Password
              </label>
              <input
                id="password"
            ...
            {error && (
              <div className="flex items-start gap-2 border border-[#e5c8c1] bg-[#f6e9e5] p-3 text-[12px] text-[#7f3a31]">
                <AlertCircle size={14} className="mt-0.5 shrink-0" />
                {error}
              </div>
            )}
            {success && (
              <div className="border border-[#d2dbc1] bg-[#f1f4e9] p-3 text-[12px] text-[#4a5e2a]">
                {success}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="vg-button vg-focus flex w-full items-center justify-center gap-2 border border-primary bg-primary py-2.5 text-[13px] font-bold text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {loading
