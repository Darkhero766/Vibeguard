...
export async function listProtectedRepositories(owner: string) { return db.select().from(protectedRepositories).where(eq(protectedRepositories.owner, owner)).orderBy(desc(protectedRepositories.updatedAt)); }
export async function listProtectionEvents(repo: string, limit = 20) { return db.select().from(protectionEvents).where(eq(protectionEvents.repo, repo)).orderBy(desc(protectionEvents.createdAt)).limit(limit); }
export async function getProtectedRepository(repo: string) { const rows = await db.select().from(protectedRepositories).where(eq(protectedRepositories.repo, repo)).limit(1); return rows[0] ?? null; }
