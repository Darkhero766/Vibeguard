...
function decodeBase64ToPem(value: string): string | null {
  ...
  try { const pem = normalizePem(Buffer.from(compact, "base64").toString("utf8")); return pem.includes("-----BEGIN") && pem.includes("PRIVATE KEY-----") ? pem : null; } catch { return null; }
}
function getPrivateKey(): string {
  const encoded = process.env.GITHUB_APP_PRIVATE_KEY_BASE64?.trim();
  if (encoded) { const decoded = decodeBase64ToPem(encoded); if (!decoded) throw new Error("GITHUB_APP_PRIVATE_KEY_BASE64 is not valid base64 PEM"); return decoded; }
...
async function completeCheckRun(token: string, repo: string, checkId: number, conclusion: "success" | "failure" | "neutral", report: any) {
  ...
  const mode = report?.scanMode === "delta" ? "\n\n**Protection mode:** only findings introduced by this change are reported." : "";
...
async function scanEvent(payload: WebhookPayload, event: "push" | "pull_request"): Promise<void> {
  ...
  if (!protectedRepo || protectedRepo.status !== "protected") {
    logger.info({ repo, event, sha }, "Ignoring event for unprotected repository");
    return;
  }
  ...
  try {
    ...
    await recordProtectionEvent({ repo, event, sha, status: conclusion, report });
    ...
    await recordProtectionEvent({ repo, event, sha, status: "failure", report: { findings: [{ severity: "Critical" }] } }).catch(() => undefined);
  }
}
