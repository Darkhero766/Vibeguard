...
async function currentSha(repoUrl: string, token?: string) {
  ...
  const refResponse = await fetch(`https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/commits/${encodeURIComponent(branch)}`, {
    headers: { Accept: "application/vnd.github+json", ...(token ? { Authorization: `Bearer ${token}` } : {}), "X-GitHub-Api-Version": "2022-11-28" },
  });
...
router.get("/protection", requireAuth, async (req: AuthedRequest, res) => {
  try {
    const repositories = await listProtectedRepositories(req.userId!);
    res.json({ repositories });
  } catch (error) {
    req.log.error({ err: error }, "Could not list protected repositories");
    res.status(500).json({ error: "Could not load protected repositories" });
  }
});
...
router.get("/protection/:repo/events", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    const events = await listProtectionEvents(repo);
    res.json({ repository: protectedRepo, events });
  } catch (error) {
    req.log.error({ err: error }, "Could not load protection activity");
    res.status(500).json({ error: "Could not load protection activity" });
  }
});
...
router.post("/protection", requireAuth, async (req: AuthedRequest, res) => {
  ...
  try {
    ...
    try {
      ...
      report = { ...report, findings: [...report.findings, ...extended.filter((f) => !seen.has(`${f.filePath}:${f.line}:${f.check}:${f.title}`))] };
